...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "mediaelection2" because\n';
  ...
  $wnd.__gwt_activeModules['mediaelection2'] = {
    'moduleName' : 'mediaelection2',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/mediaelection2.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['mediaelection2'].superdevmode = true;

  var expectedSuffix = '/mediaelection2.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + mediaelection2 + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:mediaelection2';
  var devModeSessionKey = '__gwtDevModeSession:mediaelection2';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/mediaelection2/mediaelection2.nocache.js';
  ...
  $wnd.__gwt_activeModules['mediaelection2'].canRedirect = true;
  $wnd.__gwt_activeModules['mediaelection2'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING mediaelection2';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/mediaelection2?user.agent=' + ua + '&_callback=' + callback;
